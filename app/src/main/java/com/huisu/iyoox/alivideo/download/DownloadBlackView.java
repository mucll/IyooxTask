...
import com.huisu.iyoox.alivideo.sectionlist.SectionedRecyclerViewAdapter;
...
import static com.huisu.iyoox.alivideo.download.DownloadSection.DOWNLOADED_TAG;
import static com.huisu.iyoox.alivideo.download.DownloadSection.DOWNLOADING_TAG;
 ...
 */
public class DownloadBlackView extends FrameLayout implements OnClickListener, CompoundButton.OnCheckedChangeListener {
    ...
    private DownloadBlackSection section;
    /**
     * 用于判断当前是否处于编辑状态
     */
    ...
    public DownloadBlackView(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        this.context = new WeakReference<Context>(context);
        initView();
    }
    ...
    private void initView() {
        LayoutInflater.from(this.context.get()).inflate(R.layout.alivc_download_black_view_layout, this, true);
    ...
    private void addSection(String tag, String title, final ArrayList<AlivcDownloadMediaInfo> alivcDownloadMediaInfos) {
        if (sectionAdapter.getSection(tag) == null) {
            section = new DownloadBlackSection(this.context.get(), tag, title,
                alivcDownloadMediaInfos);
            section.setOnSectionItemClickListener(new DownloadBlackSection.OnSectionItemClickListener() {
                @Override
